...
const FormTextArea: FC<FormTextAreaProps> = ({
  ...
  const { getRootProps, getInputProps } = useDropzone({
        noClick: true,
        onDrop: (files: File[]) => setAttachments([...attachments, ...Array.from(files || [])]),
  });

  return (
    <div {...getRootProps({className: "dropzone h-full w-full flex flex-col rounded-lg bg-gray py-[1dvw] px-[2dvw] lg:px-0"})} >
      <input {...getInputProps()} />
      ...
      <div className="flex flex-row px-[1dvw] text-center items-center gap-[0.5dvw] max-w-full">
        {attachments.map((file: File, index: number) => (
          <div className="flex flex-col w-1/5 h-full relative" key={index}>
            <button
              title="Remove"
              className={`w-[1.5dvw] h-[1.5dvw] top-[-0.5dvw] right-[-0.5dvw] hover:border-blue hover:text-red-600 transition-all ease-in hover:cursor-pointer absolute border-2 border-light rounded-full bg-white text-red-500 text-1.5 flex justify-center items-center text-center`}
            ...
            <div className="h-8/10 w-full">
              {(file.type === "image/gif" ||
                ...
                file.type === "image/jpg") && (
                <Image
                  className="rounded-md ease-in w-full h-[7dvw] border-2 border-light"
                  alt={"World War Community"}
                  height={0}
                  width={0}
      ...
      <div className="relative text-lightly text-2 h-full w-full flex flex-row bg-gray rounded-lg overflow-x-hidden">
        <div className="h-full w-0.5/10">
          <label
            htmlFor={"myInput"}
            className="h-full w-0.5/10 flex items-center justify-center pointer-events-none rounded-lg absolute hover:cursor-pointer"
          >
            {icon}
          </label>
          <input
        ...
        <div className="flex flex-col grow-wrap w-full">
          ...
          />
          <div
            className="replicated-value overflow-hidden h-0"
            data-replicated-value={valueProp}
          ></div>
        </div>
        ...
        <div className="h-full w-0.5/10">
          <Tippy
            content={
              <GifPicker
                ...
                onGifClick={(e: { url: string }) => setGifValue(e.url)}
              />
            }
            trigger={"click"}
            interactive={true}
          ...
          >
            <button
              type={"button"}
              className="h-full w-0.5/10 flex items-center justify-center rounded-lg"
            >
